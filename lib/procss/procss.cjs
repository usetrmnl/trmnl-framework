...
// The lookaheads exempt the contract tiers inside those namespaces: the three
// names the runtime reads off computed styles (`--tn-text-stroke-color`,
// `--tn-text-stroke-width`, `--tn-text-stroke-radius`), and the
// `--border-token-` / `--border-line-` names border-token-slot and the line
// remap resolve for tokens no shipped theme happens to use (a preserve file
// can only name the tokens its own theme touched). The runtime also builds
