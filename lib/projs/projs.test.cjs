...
const TRMNL_PAINT_METHODS = [
  ...
  'toHighchartsText',
  // Component slot resolver + MapLibre adapter (added with TRMNLMaps).
  'slot',
  'toMapLibre',
];
...
// The MapLibre composition layer. Same contract as the chart layer: these names
// are what the map docs teach, so they must survive the release minifier.
const TRMNL_MAPS_METHODS = [
  'tiles',
  'style',
  'options',
  'paint',
...
const PUBLIC_VAR_LITERALS = [
  ...
  // Component slot channel read by TRMNLPaint.slot(): the slot prefix and the
  // per-kind suffixes it projects onto the probe, plus the renderer program
  // suffix it forwards field by field.
  '--framework-slot-',
  '-bg-color',
  '-bg-image',
  '-text-color',
  ...
  '-text-under',
  '-border-render-',
  // Device capability reads behind TRMNLPaint.scale().pixel/.dither, which size
  // a MapLibre canvas and its pattern images to the device pixel grid.
  '--pixel-ratio',
  '--dither-ratio',
];
...
const PUBLIC_SELECTOR_LITERALS = [
  ...
  'title',
  'description',
  // Map attribution node TRMNLMaps.attach() writes (components/_map.scss types
  // it), the label overlay and its framework classes placeMapLabels() builds,
  // and the swatch attribute TRMNLMaps.applySwatches() paints.
  'map__attribution',
  'map__labels',
  'map__label',
  'label--small',
...
test('all three public globals survive minification', () => {
  assert.match(minified, /window\.TRMNLPaint\s*=/);
  assert.match(minified, /window\.TRMNLCharts\s*=/);
  assert.match(minified, /window\.TRMNLMaps\s*=/);
});
...
test('every TRMNLMaps public method name survives mangling', () => {
  for (const name of TRMNL_MAPS_METHODS) {
    assert.match(
      minified,
      new RegExp('[,{;]' + name + '\\s*[(:]'),
      `TRMNLMaps.${name} missing from minified output`
    );
  }
});
...
test('TRMNLCharts delegates paint resolution and conversion to TRMNLPaint', () => {
  ...
  // The chart layer ends where the map layer begins; TRMNLMaps has its own guard.
  const end = source.indexOf('\n  // ── Maps', start);
...
// The map layer is a composition layer like the chart layer: it names slots and
// assembles MapLibre style layers, and every color, pattern and ink it emits is a
// TRMNLPaint resolver result passed through toMapLibre(). No probe, no computed
// style, no tile parsing and no color literal may appear in it.
test('TRMNLMaps delegates paint resolution and conversion to TRMNLPaint', () => {
  const start = source.indexOf('\n  // ── Maps');
  const end = source.indexOf('\n  window.TRMNLPaint', start);
  assert.ok(start > 0 && end > start, 'maps section missing');
  const mapsSource = source.slice(start, end);
  assert.match(mapsSource, /TRMNLPaint\.slot\(/);
  assert.match(mapsSource, /TRMNLPaint\.toMapLibre\(/);
  assert.match(mapsSource, /TRMNLPaint\.semantic\('stroke-contrast'/);
  assert.doesNotMatch(mapsSource, /\bprobe\(|getComputedStyle|imageInk|textFillInk|normalizeHex|compositeFieldIntoTile|svgExtent/);
  assert.doesNotMatch(mapsSource, /#[0-9a-fA-F]{3,6}\b|rgba?\(/);
  assert.doesNotMatch(mapsSource, /\n\s+ink\(/);
});
...
test('3.2 documentation calls only the current paint, chart and map APIs', () => {
  ...
  for (const match of docsSource.matchAll(/TRMNLMaps\.([A-Za-z][A-Za-z0-9_]*)/g)) {
    assert.ok(TRMNL_MAPS_METHODS.includes(match[1]), `unknown documented method TRMNLMaps.${match[1]}`);
  }
});
