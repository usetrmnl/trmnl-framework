...
// The frozen v1.2 runtime. Wrapped in an IIFE so its thirteen engine declarations
// (formatValue, fitValue, pixelPerfectFonts, terminalize, …) stay private instead of
// leaking as globals that collide with plugin-authored setup/draw/formatValue or p5.js,
// the same hazard plugins.js's own IIFE header describes. The public API is the single
// window.terminalize assignment at the bottom: the 1.2 docs layouts, the test layout and
// the iframe bridge call that name and nothing else. Behavior is otherwise untouched.
(function () {
/**
  ...
  lists.forEach(list => {
    ...
    if (hiddenCount > 0 && showMoreText) {
      ...
      moreElement.innerHTML = `
        <div class="meta"></div>
        <div class="content">
          <span class="label label--gray-out">${window.I18n?.andXMore?.(hiddenCount) ?? `and ${hiddenCount} more`}</span>
        </div>
      `;
...
// The one public entry point: the 1.2 docs layouts, the test layout and the iframe
// bridge all reach the runtime through window.terminalize.
...
document.addEventListener("DOMContentLoaded", () => {
  if (window.Highcharts) {
    ...
    const interval = setInterval(() => {
      const allReady = Highcharts.charts.every(c => c && c.renderer && c.renderer.boxWrapper);

      if (allReady || counter > 50) { // 5 second timeout
    ...
    console.log('Highcharts not detected');
    window.TRMNL_HIGHCHARTS_DONE = true;
  }
});
})();