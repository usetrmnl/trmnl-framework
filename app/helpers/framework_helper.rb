...
module FrameworkHelper
  ...
  PAGE_DESCRIPTIONS = {
    ...
    'rendering_modes' => 'The grayscale tiers and color modes a screen can carry, what each one paints, and the depth it reports to the runtime',
    ...
    'themes' => 'Stylesheets that recolor a whole screen while the device still decides how colors print',
  ...
  def framework_intro_paragraphs
    @framework_intro_paragraphs ||= {
      ...
      'devices' => 'A device profile is everything the framework knows about one panel: its dimensions, pixel ratio, interface scale, size class, and density tier. Add one class, screen--{keyname}, and a screen renders with all of it. The profiles come from TRMNL\'s own device list, so each class matches a real panel.',
      'rendering_modes' => 'A rendering mode is the class that tells a screen what its panel can print. Grayscale panels pick one of three bit-depth classes; color panels pick the class for their palette. Everything else, from dither patterns to the JavaScript paint API, follows that one class.',
      ...
      'theme_authoring' => 'A theme chooses new colors for the framework\'s surfaces and changes nothing else, so a themed screen still renders the way its device requires. This page walks through the workflow: start from the boilerplate, map your slots, register the id, and lint.',
      'theme_slots' => 'This page lists every slot: each part of a screen a theme can recolor, and the mixin that sets it. A slot takes a framework token, not a raw color, so whatever you map still follows the device\'s rendering rules.',
      ...
      'chart' => 'Plugins can draw charts with any JavaScript charting library. The TRMNLCharts helper supplies the framework\'s colors, so a chart adapts to the device, dark mode, and themes like the rest of the screen.',
      ...
      'divider' => 'The Divider element provides a simple, standalone shorthand for horizontal and vertical separators. It draws the same line as the border utilities, at level 6.',
      ...
      'v3_overview' => 'Framework 3.2 makes plugins theme-aware. A theme swaps a screen\'s colors; charts, icons, borders, and strokes follow along instead of carrying their own fixed colors. Existing markup keeps working; every 3.2 feature is opt-in.',
      ...
      'v3_enhancement_guide' => 'Framework 3.2 lets an existing plugin follow themes, dark mode, and device modes everywhere: in markup, in charts, in icons, and in outlined text and images. This guide walks through the enhancements one at a time; adopt them in any order.',
