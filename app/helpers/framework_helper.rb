...
module FrameworkHelper
  ...
  def framework_intro_paragraphs
    @framework_intro_paragraphs ||= {
      ...
      'devices' => 'A device profile is everything the framework knows about one panel: its dimensions, pixel ratio, interface scale, size class, and density tier. Add one class, screen--{keyname}, and a screen renders with all of it. The profiles mirror the platform\'s device registry, so each class matches a real TRMNL panel.',
      'rendering_modes' => 'A rendering mode is the class that tells a screen what its panel can print. Grayscale panels pick one of three bit-depth classes; color panels pick the class for their palette. Everything downstream, from dither patterns to the JavaScript paint API, follows that one class.',
      ...
      'color_palettes' => 'A palette tells a screen which inks its panel can print. Four grayscale palettes cover the bit-depth tiers, and five limited color palettes dither every framework color down to the panel\'s fixed inks. On 12-bit and 24-bit panels, screen--color-full paints every color exactly as defined.',
      ...
      'themes' => 'A theme is a stylesheet you load next to plugins.css to recolor a screen. It swaps the colors the framework classes paint with, so your markup stays exactly as written. The device still renders the result its own way: dither patterns on 1-bit, the panel\'s fixed inks on limited color, exact colors on full color.',
      'paint_api' => 'TRMNLPaint is the framework\'s JavaScript API for colors. Ask it for any framework color and it returns what CSS would actually paint right now, with the device, dark mode, and the active theme already applied. Use it wherever JavaScript draws: charts, canvases, or your own rendering.',
      'paint_colors' => 'The color resolvers read the same background, text, stroke, and semantic colors your CSS classes paint with, current device, mode, and theme included. Use them when JavaScript needs the exact paint a CSS utility would produce.',
      'paint_charts' => 'The chart resolvers pick evenly spaced series colors from the framework\'s chart ramp, adjusted to the current device, mode, and theme. Adapters convert them straight into Highcharts color options.',
      'paint_borders' => 'The border resolvers read the framework border rails, the same lines the border-- utilities draw. Apply them to lines you draw yourself, or convert them for Highcharts axes and grid lines.',
      'paint_typography' => 'The typography resolver reads everything a text class sets: font, size, weight, color, and optional stroke. Apply the result to text you draw yourself, or convert it for Highcharts labels.',
      ...
      'theme_authoring' => 'A theme chooses new colors for the framework\'s surfaces and changes nothing else, so a themed screen still renders the way its device requires. This page walks the workflow: start from the boilerplate, map your slots, register the id, and lint.',
      'theme_slots' => 'This page lists every part of a screen a theme can recolor, and the mixin that sets each one. A slot takes a framework token, not a raw color, so whatever you map still follows the device\'s rendering rules.',
      'variables_api' => 'Every framework CSS variable is either public contract or private implementation. This page draws that line: which variable families you can rely on, and which are internal and free to change. It also shows how the Paint API, themes, and the Sass source each use the public ones.',
      ...
      'inverse' => 'The Inverse utility flips one element and its children to the opposite color scheme: light content on a dark surface, or back. Use it to make one element stand out, an active item or a selected row, without touching its siblings.',
      ...
      'v3_overview' => 'Framework 3.2 makes plugins theme-aware. A theme swaps a screen\'s colors, and charts, icons, borders, and strokes follow along instead of carrying their own fixed colors. Existing markup keeps working; every 3.2 feature is opt-in.',
      ...
      'v3_enhancement_guide' => 'Framework 3.2 lets an existing plugin follow themes, dark mode, and device modes everywhere: in markup, in charts, in icons, and in outlined text and images. This guide walks the enhancements one at a time; adopt them in any order.',
