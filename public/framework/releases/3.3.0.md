...
`relative` and `absolute` lift one element out of flow and place it against the nearest positioned ancestor, with `inset--{size}`, `top--{size}`, `right--{size}`, `bottom--{size}` and `left--{size}` on the spacing scale and a `z--0` to `z--3` stacking scale, every one of them carrying the size and orientation variants the rest of the box model carries. A screen can now give a map or an image the whole layout and float a compact card over one corner of it, instead of splitting into columns and paying for the map twice. Positioned content sits outside the terminalize budgets and carries its own sizing and clamps; the Layout and Map pages state that, and the paint an overlay card needs to stay readable on 1-bit.
...
- Added Positioned Content to the Layout page (the class list, the overlay demo, and the budget rule) and the Floating card pattern to the Map page, with the fill, hairline and shadow rules a card over a map has to follow.
