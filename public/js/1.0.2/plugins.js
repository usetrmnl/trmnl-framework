...
function formatValue() {
  ...
  valueElements.forEach(element => {
    ...
    // Store raw value in dataset only if not already set
    ...
    if (isNaN(number)) {
      console.warn('Invalid number:', rawValue);
      return;
    }
    ...
    // Find best format and update element once
    const formattedValue = findBestFormat(number, availableWidth, element);
    console.log('Formatted value:', formattedValue);
    element.textContent = formattedValue;

    // Only call fitTextToContainer if explicitly requested
