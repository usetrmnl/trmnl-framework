...
function getScreenContext() {
  ...
  if (screen) {
    ...
    // Bit depth (grayscale and color palettes)
    ...
    else if (screen.classList.contains('screen--color-full')) bitDepth = 12;
    else if (screen.classList.contains('screen--color-7a') ||
             screen.classList.contains('screen--color-6a') ||
             screen.classList.contains('screen--color-4bwry') ||
             screen.classList.contains('screen--color-3bwr') ||
             screen.classList.contains('screen--color-3bwy')) bitDepth = 4;
...
// Matches arbitrary pixel-gap utility classes like `gap--[10px]` plus their
// responsive prefixes (e.g. `lg:gap--[12px]`, `dark:2bit:gap--[40px]`). When
// the user wrote an exact pixel value we treat it as immutable and skip the
// pixel-perfect adjustment entirely.
const ARBITRARY_GAP_CLASS_RE = /^(?:[a-z0-9-]+:)*gap--\[\d+(?:\.\d+)?(?:px)?\]$/;
...
function readComputedHorizontalGap(el, properties, fallbackMeasure) {
  ...
  if (gap === 0 && typeof fallbackMeasure === 'function') {
...
function adjustGapForIntegerColumns(el, columnCount, label, measureGap) {
  ...
  if (hasArbitraryGapClass(el)) {
    ...
    console.log(`${label} gap adjustment: skipped (arbitrary gap--[Npx] class)`);
    return;
  }
  ...
  console.log(`${label} gap adjustment: Container ${containerWidth}px, ${columnCount} columns, intended gap ${intendedGap}px`);
  ...
  if (!candidate) {
    clearAdjustedGap(el);
    console.log(`${label} gap adjustment: skipped (no candidate within tolerance of ${intendedGap}px)`);
    return;
  }
  ...
  console.log(`Adjusted ${label.toLowerCase()} gap from ${intendedGap}px to ${candidate.bestGap}px, resulting in ${candidate.bestColumnWidth}px columns`);
}
  ...
  containers.forEach(container => {
    ...
    adjustGapForIntegerColumns(container, columnCount, 'Column', () => (
      readComputedHorizontalGap(container, ['columnGap', 'gap'], () => {
        ...
        return secondColumn.left - firstColumn.right;
      })
    ));
  });
}
...
function addClassBasedOnHeight(element, height, threshold) {
  ...
  if (children.length === 0) {
    ...
    console.log(`Content limiter: clamped text-only content to ${maxLines} lines (usable=${usablePx}px, line=${lineHeightPx}px).`);
    return;
  }
...
async function terminalize() {
...

  {
    ...
    // Re-clamp after layout (e.g. grid cells get real width) so path-like and grid content truncates correctly
    ...
    tables.forEach((table) => {
      try {
        ...
        const showLabel = (function () {
          const counter = table.getAttribute('table-overflow-counter');
  ...
  // Final pass: adjust column gaps after Overflow engine has settled columns
  {
    ...
    // This is in a finally block to ensure it's set even if terminalize() errors
...
// Version marker: edit this string and hard-refresh; if the UI/console do not update,
// you are not running this file (e.g. pinned /js/<ver>/plugins.js or a stale asset).
window.__TRMNL_BUILD__ = 'plugins.js v2026-07-17-2';
