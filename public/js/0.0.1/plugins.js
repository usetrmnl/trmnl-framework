...
function manageOverflow() {
  ...
  lists.forEach(list => {
    ...
    // Remove any existing "more-items" element
    const existingMoreElement = list.nextElementSibling;
    if (existingMoreElement && existingMoreElement.classList.contains('more-items')) {
      existingMoreElement.remove();
    }
    ...
    if (hiddenCount > 0 && showMoreText) {
      ...
      `;
      list.parentNode.insertBefore(moreElement, list.nextSibling);
    }
  });
}
